          ...
          {categories.data.map(category => (
            <li key={categories.id} class="main-product-menus">
              <div class="menus-product-wrap">
                ...
                <a href={"http://localhost:3000/categories/" + categories.id}></a>
              </div>
            </li>
          ))}
        </div>
...
Home.getInitialProps = async (context) => {
  ...
  try {
    ...
    const headers = {
      ...
      const sokovi = await fetch('http://localhost:1337/api/products?populate=*&filters[categories][id][$eq]=1', {
        method: 'GET',
        headers,
      })
